...
export default function NewJournalPage() {
  ...
  useEffect(() => {
    ...
    api.get('/tags').then(d => setTags(d.tags)).catch(() => {});
  }, []);
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <textarea
          ...
          className="w-full px-6 py-4 min-h-[400px] outline-none resize-none text-gray-700 leading-relaxed"
        />
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map(tag => (
                <button key={tag._id} onClick={() => setSelectedTags(prev => prev.includes(tag._id) ? prev.filter(id => id !== tag._id) : [...prev, tag._id])} className={`px-3 py-1 rounded-full text-sm border transition-colors ${selectedTags.includes(tag._id) ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                  {tag.name}
                </button>
              ))}
            </div>
